...
def fit_layout(floor_xz, need_cube=False, show=False, block_eps=0.05):
    ...
    res = cv2.findContours(floorplan, 1, 2)
    # Xử lý cho cả OpenCV 3 và 4
    polys = res[1] if len(res) == 3 else res[0]
    # Quan trọng: Ép kiểu về list để có thể sử dụng .sort()
    polys = list(polys) 
    ...
    # Tìm đoạn xử lý pred_polys
    pred = np.uint8(ans)
    res_pred = cv2.findContours(pred, 1, 3)
    pred_polys = res_pred[1] if len(res_pred) == 3 else res_pred[0]
    # Ép kiểu về list trước khi sort
