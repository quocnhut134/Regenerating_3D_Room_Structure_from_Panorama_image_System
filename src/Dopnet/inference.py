...
def save_raw_boundary_json(image_name, output_xyz, ratio, img_shape, save_path):
    h, w = img_shape[:2]

    floor_uv, ceil_uv = corners2boundaries(
        ratio,
...
def run_one_inference(img, model, args, name):
    ...
    # JSON raw giống format LGT mẫu
    save_raw_boundary_json(
        image_name=f"{name}.png",
        output_xyz=output_xyz,
        ratio=ratio,
        img_shape=img.shape,
        save_path=os.path.join(args.output_dir, f"{name}_raw.json")
    )
...
if __name__ == '__main__':
    ...
    # Force CPU because current PyTorch is CPU-only
    args.device = "cpu"
    config.defrost()
    ...
    model.load(args.device, logger, best=True)
