...
def visualization(segments: np.array, output_polygon: np.array, center: np.array, side_l=1000):
    ...
    board = cv2.drawMarker(board, tuple(origin.astype(int)), 1, thickness=3)

    output_polygon = (output_polygon - origin) * scale + origin
    board = cv2.drawContours(board, [output_polygon.astype(int)], 0, 1, 3)
    board = cv2.drawMarker(board, tuple(origin.astype(int)), 1, thickness=3)
    plt.axis('off')
    plt.imshow(board)
    plt.show()
...
if __name__ == '__main__':
    ...
    mp3d_dataset = MP3DDataset(root_dir='/data/cylin/zzs/Datasets/mp3d', mode='train',
    ...
    img = draw_iou_floorplan(gt_xz=uv2xyz(gt['corners'])[..., ::2],
                             dt_xz=calc_visible_polygon(np.array([0, 0]), uv2xyz(gt['corners'])[..., ::2], show=True),
    ...
    result.save('/data/cylin/zzs/Pan_layout/LGT-Net/src/fig/sample3.png')
